...
let feature_movie = document.querySelector(".featured")
function displayMovie(movie){
    feature_movie.innerHTML = `
    <div class="card">
        <div class="card-header">${movie.title}</div>
        <img src="${movie.poster}" class="card-img-top" alt="${movie.title}">
        <div class="card-body">
            <h5 class="card-title"><small>${movie.year}, </small>${movie.genre}</h5>
...
function searchMovies(event){
    ...
    feature_movie.innerHTML = count == 0 ? 'Nothing was found' : ''
}
